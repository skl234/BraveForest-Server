...
존·포탈·지형·스폰은 파일에서 읽지만 몬스터 능력치·플레이어 성장값·캐릭터 정보는 기존 DB를 계속 사용한다. DB 테이블은 이번 정리에서 삭제하지 않았다.
...
2026-09-07 소스 정리 후 새 Release 로그인·마을·비기너존 EXE의 INI/DB 초기화, 각 포트 Listen, Q 정상 종료를 확인했다. 테스트 플레이어는 접속시키지 않았고 계정·캐릭터 데이터는 변경하지 않았다.

서버 빌드 및 전투/채팅 회귀 테스트 결과는 Build/Validation에 보관한다. 과거 캡처와 복구 자료는 Build/LocalArchive에 있으며 현재 실행 설명에 사용하지 않는다.
